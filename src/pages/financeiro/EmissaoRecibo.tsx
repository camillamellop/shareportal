...
import { Receipt, Download, Eye, Plus, Save, CheckCircle, FileText } from "lucide-react";
...
export default function EmissaoRecibo() {
  ...
  const loadRecibos = async () => {
    // Simular carregamento de recibos (integrar com Firebase depois)
    setRecibosEmitidos([]);
  };
  ...
  const generateNextNumber = () => {
    ...
    const nextNumber = String(recibosEmitidos.length + 1).padStart(3, '0');
    setFormData(prev => ({ ...prev, numero: `${nextNumber}/${year}` }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Simular criação do recibo (integrar com Firebase depois)
  ...
  const generateReciboPDF = async () => {
    try {
      await PDFGenerator.generateReciboPDF({
        ...
        descricao: formData.descricao,
        data: formData.data,
        forma_pagamento: 'Dinheiro' // Pode ser ajustado conforme necessário
      });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              Gere recibos e crie automaticamente despesas para ressarcimento
            </p>
          </div>
        </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="valor-extenso">Valor por Extenso</Label>
                            <Input 
                        id="valor-extenso" 
                        value={valorPorExtenso(formData.valor)}
                        readOnly
                        className="bg-muted"
                      />
                    </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                      ...
                      <p className="text-sm text-blue-700">
                        Ao salvar este recibo, será criada automaticamente uma <strong>despesa pendente</strong> para o cliente, 
                        iniciando o processo de ressarcimento.
                      </p>
                    </div>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Button 
                                ...
                                size="sm"
                                onClick={() => {
                                  // Aqui você pode implementar a geração de PDF para recibos já emitidos
            ...
            <div className="bg-white p-8 border rounded-lg" style={{ fontFamily: 'Arial, sans-serif' }}>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1 text-left">
                      ...
                      <div className="text-sm text-gray-700 space-y-1">
                        <p><strong>CNPJ:</strong> {configEmpresa.cnpj} {configEmpresa.inscricaoMunicipal && `• Inscrição Municipal: ${configEmpresa.inscricaoMunicipal}`}</p>
                        <p><strong>Endereço:</strong> {configEmpresa.endereco}, {configEmpresa.cidade} - {configEmpresa.estado}, {configEmpresa.cep}</p>
              ...
              <div className="text-center mt-8">
                <p className="text-sm text-gray-600">
                  {configEmpresa?.cidade}, {new Date(formData.data).toLocaleDateString('pt-BR', { 
                    day: 'numeric', 
                    month: 'long', 
                    year: 'numeric' 
                  })}
